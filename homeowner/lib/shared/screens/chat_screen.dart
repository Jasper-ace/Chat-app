...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Future<void> _initializeCurrentUserId() async {
    try {
      // Debug: Check what Firebase user is logged in
      ...
      print('🔍 DEBUG: Retrieved autoId: $autoId');
  ...
  /// Helper method to get the common IDs needed for state services
  Map<String, dynamic> _getConversationParams() {
  ...
  Future<bool> _isConversationMuted() async {
    final params = _getConversationParams();
    if (params['otherUserId'] == null) return false;
    return await ConversationStateService.isConversationMuted(
      currentUserId: params['currentUserId'] as int,
      currentUserType: params['currentUserType'] as String,
      otherUserId: params['otherUserId'] as int,
      otherUserType: params['otherUserType'] as String,
    );
  }

  // Check for archiving status
  Future<bool> _isConversationArchived() async {
    final params = _getConversationParams();
    if (params['otherUserId'] == null) return false;
  ...
  // --- START Unified Thread Logic ---
  // Thread ID generation is now handled entirely by UnifiedThreadService
  // --- END Unified Thread Logic ---
  ...
  void _markConversationAsRead() {
    try {
      if (widget.otherUser.autoId != null) {
        final otherUserType = widget.currentUserType == 'homeowner'
            ? 'tradie'
            : 'homeowner';
  ...
  // Simplified message bubble - no need for thread ID since message already contains all data
  Widget _buildMessageBubble(MessageModel message, bool isMe) {
    return _buildMessageContent(message, isMe);
  }

  // Core message content builder
            ...
            child: Row(
              ...
              children: [
                ...
                if (isMe) ...[
                  ...
                  // Assuming MessageStatusService.getStatusIcon is implemented correctly
  ...
  void _showChatOptions() {
    showModalBottomSheet(
      ...
      builder: (context) {
        // Use FutureBuilder to fetch dynamic states (muted, archived)
        return FutureBuilder<List<bool>>(
          future: Future.wait([
            _isConversationMuted(),
            _isConversationArchived(),
          ]),
          builder: (context, snapshot) {
            final isMuted = snapshot.data?[0] ?? false;
            final isArchived = snapshot.data?[1] ?? false;
            ...
            return Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // 2. Mute/Unmute Notifications (Functioning)
                  ListTile(
                    ...
                    onTap: () {
                      _toggleMute(isMuted: isMuted);
                    },
                  ),
                  // 3. Archive/Unarchive Chat (Functioning)
                  ListTile(
                  ...
                  // 6. Report User (Placeholder/Mock)
                  ListTile(
  ...
  void _toggleMute({required bool isMuted}) async {
    if (widget.otherUser.autoId == null) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Cannot mute: User ID not available')),
        );
      }
      return;
    }
    ...
    try {
      ...
      if (isMuted) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('${widget.otherUser.name} unmuted')),
          );
        }
      } else {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('${widget.otherUser.name} muted')),
          );
        }
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Error toggling mute: $e')));
      }
    }
  }

  void _toggleArchive({required bool isArchived}) async {
    if (widget.otherUser.autoId == null) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Cannot archive: User ID not available'),
          ),
        );
      }
      return;
    ...
    try {
      ...
      if (isArchived) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('${widget.otherUser.name} unarchived')),
          );
        }
      } else {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('${widget.otherUser.name} archived')),
          );
        }
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Error toggling archive: $e')));
      }
    }
  }
