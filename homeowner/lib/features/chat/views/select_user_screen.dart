...
class _SelectUserScreenState extends ConsumerState<SelectUserScreen> {
  ...
  Future<void> _loadUsers() async {
    ...
    try {
      ...
      // Fetch all tradies from Laravel API
      final response = await http.get(
        Uri.parse('http://10.0.2.2:8000/api/tradies'),
        headers: {
  ...
  Future<void> _startChat(Map<String, dynamic> tradie) async {
    try {
  ...
  List<Map<String, dynamic>> get _filteredUsers {
    ...
    return _users.where((user) {
      ...
      final businessName = (user['business_name'] ?? '').toLowerCase();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('Select Tradie', style: AppTextStyles.appBarTitle),
      ),
      body: Column(
        children: [
          // Search bar
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text(
            _searchQuery.isEmpty ? 'No tradies found' : 'No results',
  ...
  Widget _buildUserItem(Map<String, dynamic> user) {
    final name = user['first_name'] ?? 'Unknown';
